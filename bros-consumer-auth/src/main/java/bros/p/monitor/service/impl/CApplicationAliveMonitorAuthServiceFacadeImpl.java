...
/**
 * 
 * @ClassName: CApplicationAliveMonitorAuthServiceFacadeImpl 
 * @Description: 监听bros-consumer-auth模块是否存活
 * @author 何鹏
 * @date 2016年12月19日 下午2:13:27 
 * @version 1.0
 */
@Component("capplicationAliveMonitorAuthServiceFacade")
public class CApplicationAliveMonitorAuthServiceFacadeImpl implements
		ICApplicationAliveMonitorAuthServiceFacade {
	private static final Logger logger = LoggerFactory.getLogger(CApplicationAliveMonitorAuthServiceFacadeImpl.class);
	@Autowired
	private IPApplicationAliveMonitorAuthServiceFacade papplicationAliveMonitorAuthServiceFacade;
	@Autowired
	...
	public List<Map<String,Object>> applicationAliveMonitorMethod(
		...
		String modelName = "bros-consumer-auth";
		...
		try{
			list.addAll(papplicationAliveMonitorAuthServiceFacade.applicationAliveMonitorMethod(headMap, bodyMap));
		}catch(Exception e){
			logger.info("检查应用是否存活，不影响交易",e);
			Map<String,Object> callMap = new HashMap<String, Object>();
			callMap.put("returnCode", "0001");
			callMap.put("returnMsg", "状态不正常");
			callMap.put("modelName", "bros-provider-auth");
			list.add(callMap);
		}
		return list;
	}
